...
/* A APRESENTAÇÃO DELA
   ...
   rápido no celular e sem depender de leitor de PDF. */
function viewApresentacao() {
  const paginas = DB.settings.apresentacao || [];
  app.innerHTML = `
  <header class="topbar">
    <button class="backbtn" id="bk" aria-label="${t('back')}">←</button>
  ...
  <main class="wrap apre">
    <h1 class="pageh">${t('apreTit')}</h1>
    <p class="desc lead">${t('apreSub')}</p>
    ${paginas.map((p, i) => `<img class="apre-pg" src="${esc(p)}" alt="${t('apreTit')} ${i + 1}" loading="lazy">`).join('')}
    <div class="apre-fim">
      <a class="cta" href="${waLink(t('waHello'))}" target="_blank" rel="noopener">${t('whatsapp')}</a>
      <button class="cta ghost" id="verTours">${t('seeTours')}</button>
    </div>
  </main>`;
  bindLang(app);
  $('#bk').onclick = () => go('/');
  $('#verTours').onclick = () => { viewShowcase._f = 'all'; go('/tours'); };
}
...
const TYPE_LABEL = { day: 'fDay', walk: 'fWalk', photo: 'fPhoto', session: 'tSession', bike: 'fBike',
                     carro: 'fCarro', transfer: 'fTransfer' };
...
function viewShowcase() {
  ...
  <main class="wrap">
    ...
    <div class="chips" id="filters">
      ${['all', 'walk', 'carro', 'bike', 'day', 'transfer', 'session'].filter(f => f === 'all' || Tours.live().some(x => x.type === f)).map(f =>
      ...
      ${list.length ? list.map(x => `
        <button class="tourcard" data-id="${x.id}">
          ...
          <span class="bd">
            ...
            <span class="cardfoot">
              <span class="pr">${!+x.price ? t('priceAsk') : `${x.priceLate && x.earlySeats && x.priceMode !== 'session' ? `<u>${t('fromPrice')}</u> ` : ''}${eur(x.price)}
  ...
  <main class="wrap two-col">
    <section class="tourbody">
      ...
      <div class="sec">
        ...
        <div class="pricebox">
          <div class="pbmain">
            ${!+x.price ? `<b>${t('priceAsk')}</b><small>${t('priceAskWhy')}</small>`
              : `<b>${eur(x.price)}</b><small>${x.priceMode === 'session' ? t('perSession') : t('perPerson')}</small>`}
            ${linhaReais(x.price)}
...
function renderBook() {
  ...
  const priceLine = x.priceMode === 'session'
    ? (!+x.price ? t('priceAsk') : `${eur(x.price)} <small>${t('perSession')}</small>`)
    : (!+valorEmDestaque ? t('priceAsk') : `${eur(valorEmDestaque)} <small>${t('perPerson')}</small>`)
