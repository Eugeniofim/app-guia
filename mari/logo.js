...
/* simbolo — para icones, selos e cabecalhos.
   Se o guia tiver selo proprio (APP_CONFIG.guia.logo), usamos o dele; senao,
   o anel com a inicial do negocio. O codigo nao carrega marca de ninguem. */
function logoMark(height = 40, color = '#FFD23F', opts = {}) {
  const cls = opts.cls ? ` class="${opts.cls}"` : '';
  const selo = (typeof APP_CONFIG === 'object' && APP_CONFIG && APP_CONFIG.guia && APP_CONFIG.guia.logo) || '';
  if (selo) {
    return `<img${cls} src="${_lgEsc(selo)}" width="${height}" height="${height}" alt=""
